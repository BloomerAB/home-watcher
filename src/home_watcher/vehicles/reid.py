"""Vehicle Re-Identification using ResNet18 embeddings."""
...
class VehicleReID:
    ...
    def match(self, embedding: np.ndarray) -> VehicleMatch:
        if not self._embeddings:
            return VehicleMatch(matched_subject=None, similarity=0.0)
        ...
        if best_sim >= self.threshold and best_subject:
            return VehicleMatch(matched_subject=best_subject, similarity=best_sim)
        return VehicleMatch(matched_subject=None, similarity=best_sim)
